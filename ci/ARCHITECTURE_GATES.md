...
- **Local alternative:** At Level 1–2, a short comment in the commit message or a one-paragraph note in `notes/` is sufficient. A formal ADR is not required until Level 3.
- **Cost:** Medium. ADRs take time to write and maintain. Do not require them for reversible or small-scope decisions.
- **Failure action:** At Level 3+, block merge until an ADR is present in `adr/`. At Level 2, warn but do not block.
...
- **Local alternative:** At Level 1–2, the developer (or AI agent) states the blast radius in the commit message. A dependency graph diff tool is not required.
- **Cost:** Medium. Dependency graph diffing requires tooling investment. At Level 2, a manual declaration is acceptable.
- **Failure action:** At Level 3+, block merge if blast radius has increased without justification. Require a declared scope list for changes spanning more than one logical slice.
